...
class ImagePage(tk.Frame):
    ...
    def load_file(self):
        ...
        try:
            filenames = askopenfilenames(initialdir=application_path, filetypes=(
            ("DICOM Files", "*.dcm"), ("JPEG Files", "*.jpg"), ("TIF Files", "*.tif"), ("PNG Files", "*.png"),
            ("All Files", "*.*")), title="Open Image File")
            filenames = list(filenames)

            if not filenames:
                return

            filename_suffix = Path(filenames[0]).suffix
            for filename in filenames:
                if filename.endswith(filename_suffix):
                    continue
                else:
                    raise imgErrs.NotSameFileTypeError

            self.pixels = []
            if filename_suffix == '.dcm':
            ...
            for filename in filenames:
                ...
                if filename_suffix == '.dcm':
                    ...
                    if ds.StudyInstanceUID != studyUID:
                        raise imgErrs.DcmStudyError

                    image = np.asarray(ds.pixel_array)
                else:
                    # convert non-dicom image to grayscale so that it can be analysed as a 2D array in just 2D
                    image = PIL.Image.open(filename).convert('L')
                    image = np.asarray(image, dtype="int16")

                # raise error if not a 2 or 3d pixel array
                if image.ndim < 2 or image.ndim > 3:
                    raise imgErrs.DimError

                # handle a 3D pixel_array from a single file, e.g. a RD dose cube
                if image.ndim > 2:
                    for im in image:
            ...
            messagebox.showinfo(title="Unable to Load File",
                                message="Unable to load file, please choose another file or consult the debug log. Error: " + str(e))
    ...
    def get_results(self, method):
        ...
        try:
            ...
            messagebox.showinfo(title="Unable to Perform Analysis",
                                message="Unable to perform analysis, ensure a file is loaded or consult the debug log.")
            logger.error(e, exc_info=True)
    ...
    def profile_coords(self, event):
        ...
        if len(self.coords) > 3:
            ...
            try:
                ...
                messagebox.showinfo("Profile Results",
                                    "Hor F: " + str(horf) + " Hor S: " + str(hors) + " Ver F: " + str(
                                        verf) + " Ver S: " + str(vers))

            except Exception as e:
                messagebox.showinfo(title="Unable to Calculate Profiles",
                                    message="Unable to calculate profiles, please try again in another area or consult the debug log.")
                logger.error(e, exc_info=True)
